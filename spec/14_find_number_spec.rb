...
describe FindNumber do
  ...
  describe '#game_over?' do
    ...
    context 'when guess and random_number are not equal' do
      let(:number) { double('random_number', value: 5) }
      subject(:game_continue) { described_class.new(0, 9, number, 4) }
  ...
  describe '#update_range' do
    ...
    context 'if all values are the same' do
      subject(:game) { described_class.new(10, 10, number_range, 10)}
  
      before do
        game.update_range
      end
  
      it 'min doesnt update' do
        min = game.min
        expect(min).to eq(10)
      end
    ...
    context 'if guess equals the minimum' do
      subject(:game) { described_class.new(3, 10, number_range, 3)}

      before do
        game.update_range
      end

      it 'min still updates' do
        min = game.min
        expect(min).to eq(4)
      end
    end
